...
class RequestTests: XCTestCase {
    ...
    private func verifyRequest<T: Response<U>, U>(_ expected: String, _ makeRequest: (NeoSwift) throws -> Request<T, U>) async {
    ...
    private func verifyExpressRequest<T: Response<U>, U>(_ expected: String, _ makeRequest: (NeoSwiftExpress) throws -> Request<T, U>) async {
        let mockUrlSession = MockURLSession().requestInterceptor { request in
            guard let body = request.httpBody else {
                return XCTFail("No request body")
            }
